...
export interface FillContext {
   ...
   *
   * Пусто — вопрос про каталог вообще, и отвечает страница. Непусто — вопрос
   * про конкретный товар, и ответ обязан быть про него: клиент, спросивший про
   * рюкзак и получивший кофе, делает вывод, что по рюкзаку данных нет.
   */
  selection: readonly Product[]
  ...
  /**
   * Весь журнал, без отбора по окну.
   *
   * Нужен аналитике: показатели дня считаются по событиям этого дня, а окно
   * запроса к делу не относится — оно решает, какие дни показать, а не какие
   * заказы в них были.
   */
