...
| `call-actor` | вход проверяется по схеме актора, выдача — по схеме его датасета |
| `get-actor-run` | мок метода `GET /v2/actor-runs/{runId}` |
| `get-dataset-items` | мок метода `GET /v2/datasets/{datasetId}/items` |
...
`get-dataset-items` через MCP и `GET /apify/v2/datasets/{id}/items` через шлюз
отдают одно и то же тело, а не два разных выдуманных.
...
а не после платного запуска. Выдача строится из описания полей датасета, где
значения берутся из `examples`, которые написал сам автор актора.
