...
Четыре метода не просто отвечают успехом, а меняют то, что видит следующий
запрос той же песочницы, — как в бою:

- `POST /content/v2/cards/update` — название, описание, характеристики.
  Проверяются `nmID`/`vendorCode`/`sizes` (обязательны), длина названия
  (60 символов) и описания (5000), и характеристики — по небольшому
  справочнику песочницы (`14177449` «Цвет» и `14177450` «Состав» — настоящие
  ID из спецификации; полного каталога характеристик по каждому из тысяч
  предметов WB стенд не ведёт). `GET /content/v2/object/charcs/{subjectId}`
  отдаёт этот же справочник — только характеристики запрошенного предмета,
  с настоящими именами («Цвет», а не название товара), теми же ID, что
  примет `cards/update`.
- `POST /api/v2/upload/task` — цена и скидка. Отвечает `id` загрузки сразу
  же обработанным (`status: 3` в `GET /api/v2/history/tasks`) — песочница
  не моделирует очередь обработки.
- `POST /content/v3/media/save` и `POST /content/v3/media/file` — фото.
  Ссылки сохраняются как есть; у файла, загруженного вторым методом,
  реального содержимого нет — только адрес, детерминированный по артикулу
  и номеру.
