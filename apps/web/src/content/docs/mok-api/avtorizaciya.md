...
:::note Ключ открывает все сервисы
Деления ключа по сервисам нет: любой активный ключ песочницы работает со всеми
сервисами стенда, включая те, что появятся позже. Раньше набор задавался при
создании и потом не менялся — и ключ, выданный до появления нового сервиса,
оставался без него навсегда.
:::
