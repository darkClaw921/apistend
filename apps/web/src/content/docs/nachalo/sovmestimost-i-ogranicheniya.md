...
**Ответ несёт лишние заголовки.** `X-APIStend-Source`, `-Readiness`, `-Scenario`,
`-Upstream`, `-Snapshot`, `X-Request-Id`, а на 404 — ещё и подсказка
`X-APIStend-Did-You-Mean` с похожими путями каталога. В бою ничего этого нет.
Так и задумано: мок обязан быть неотличим по телу и коду ответа и отличим
по метаданным.
...
- [Заголовки честности](/docs/mok-api/zagolovki-chestnosti) — разбор метаданных ответа
