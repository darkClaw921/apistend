...
Вторая песочница нужна тогда, когда работ две: одна под отлаживаемую интеграцию,
другая под демонстрацию или под автотесты, которые сбрасывают данные. Ключи,
вебхуки, свои моки и журнал запросов принадлежат песочнице и между ними
не переносятся.
...
через [Management API](/docs/api-upravleniya/resursy-pesochnicy).
...
:::note Доля ошибок выключена по умолчанию
Свежая песочница не отдаёт случайных сбоев. Включите долю ошибок, когда будете
проверять, как ваш код переживает 500: случайность здесь настоящая, а не
детерминированная, — иначе на одном и том же вызове сбой либо не наступал бы
никогда, либо наступал всегда.
