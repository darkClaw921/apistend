...
Регистрация — почта и пароль от восьми символов; подтверждения почты и входа
через сторонние провайдеры нет. Имя и название проекта необязательны: без имени
подставляется часть адреса до собаки, без проекта — «Первый проект».
...
curl -s -X POST localhost:8080/api/auth/register \
  ...
  -d '{"email":"you@example.com","password":"apistend2026","project":"Интеграция 1С"}'
```

```json
{
  "user": {"id":"cmttxymew0007…","email":"you@example.com","name":"You","initials":"Y"},
...
на все три сервиса: без них продуктом нельзя пользоваться, поэтому отдельного
